...
from philogenetic_trees.io import load_from_file, load_from_string, write_to_file
...
class PhiTree:
    ...
    def _get_leaves(tree: List[Node]) -> List[str]:
        ...
        leaves = list(filter(lambda v: not PhiTree._is_group(v), vertices))
        if leaves is []:
    ...
    def _check_group(root: Node):
        if not PhiTree._is_group(root.name):
            raise ValueError(f"{root.name} is not a group")
        children = set(root.descendants)
        leaves = PhiTree._get_group_leaves(root.name)
        expected_leaves = set(leaves)
        known_leaves = set(leaves[:])

        for ch in children:
            ch_name = ch.name
            if PhiTree._is_group(ch_name):
                ch_leaves = set(PhiTree._get_group_leaves(ch_name))
            ...
            elif ch_name in expected_leaves:
    ...
    def _check_next_node(node: Node):
        if PhiTree._is_group(node.name):
            PhiTree._check_group(node)
            for child in node.descendants:
                PhiTree._check_next_node(child)

    @staticmethod
    def check_tree(tree: List[Node]) -> bool:
        try:
            PhiTree._check_next_node(tree[0])
...
def _get_nodes(node: Node):
    ...
    for child in node.descendants:
        ...
        child_vertices, child_edges = _get_nodes(child)
